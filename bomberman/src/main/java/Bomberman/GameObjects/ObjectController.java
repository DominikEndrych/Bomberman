...
public class ObjectController {
    ...
    /*
    private boolean canMoveHere(Enemy enemy, int x, int y){
        
        if(enemy.getX() + x - 40 < 0 || enemy.getX() + x + 40 > this.displayWidth || enemy.getY() + y - 40 < 0 || enemy.getY() + y + 40 > this.displayHeight){
            return false;
        }
        
        if(x != 0 && x < 0){
            if(this.map.checkMatrix((enemy.getY() + y) / 80, (enemy.getX() + x - 40) / 80) != 1){
                return true;
            }
            else{
                //System.out.println("Nemuzu");
                return false;
            } 
        }
        ...
        if(y != 0 && y < 0){
            if(this.map.checkMatrix((enemy.getY() + y - 40) / 80, (enemy.getX() + x) / 80) != 1){
                return true;
            }
            else{
                //System.out.println("Nemuzu");
    ...
    */
    ...
    public void moveEnemies(Enemy enemy){
       ...
       // while(iterator.hasNext()){
            ...
            if( (enemy.getX() - 40) % 80 == 0 && (enemy.getY() - 40) % 80 == 0){
                osa = rand.nextInt(2);
                smer = rand.nextInt(2);
